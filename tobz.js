...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6283164445159@c.us'
        ...
        // FUNCTION
                ...
                if (chats.match("anjing") || chats.match("gblk") || chats.match("tolol") || chats.match("kntl")) {
                    if (!isGroupAdmins) {
                        ...
                        }).catch(() => tobz.sendText(from, `Untung REV Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                    } else {
                        return tobz.reply(from, "Tolong Jaga Ucapan Yahh 😇", id)
                    }
                }
                
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin REV!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner REV!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner REV!', id)
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `6283164445159@c.us`)
            tobz.reply(from, 'Itu nomor Owner Ku', id)
            break
        // ON OFF
        case '#nsfw':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                tobz.reply(from, 'Pilih enable atau disable Cok!', id)
            }
            break
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin REV!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case '#ytmp4':
            ...
            break
        case '#play':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin REV!`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
        ...
        case '#nhentai': // SEARCH NHENTAI
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6283164445159', id)
          break
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner REV`, id)
                bctxt = body.slice(4)
                txtbc = `*「 REV BROADCAST 」*\n\n${bctxt}`
        ...
        case '#ownergroup':
            ...
            break
        case '#otagall': // FOR OWNER & ADMIN REV
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner REV', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin REV', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin REV', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin REV', id)
            await tobz.sendText(from,'REV DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#REVadmin':
            let admn = `This is list of REV Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner REV!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin REV!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner REV!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin REV!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner REV!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#help':
            ...
            break
        case '#REVgroup':
            tobz.reply(from, `Link Group REV : https://chat.whatsapp.com/KBt8lh74kvUJ6gunrVkco4\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin REV', id)
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup REV, Link Ada Di Bio atau Bisa Mengetik #REVgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
